...
export function LeftInventory({ inventory }: { inventory: InventoryItem[] }) {
    ...
    const handleMouseUp = (targetSlot: number) => {
        if (draggedItem && isDragging) {
            console.log(`Swapping item from slot ${draggedItem.slot} to slot ${targetSlot}`);
    ...
    return (
        ...
        >
            <Group gap="4px" mt="5vh" align="flex-start">
                <Box
                    w="50px"
                    h="50px"
                    style={{
                        ...
                        {slots.map((slotIndex) => {
                            ...
                            return (
                                <Box
                                    ...
                                    onMouseEnter={item && !isDragging ? (e) => handleMouseEnter(item, e) : undefined}
                                    onMouseMove={undefined} // Remove individual mouse move during dragging
                                    onMouseLeave={item && !isDragging ? handleMouseLeave : undefined}
                                    onMouseDown={item ? (e) => handleMouseDown(item, e) : undefined}
                                    onMouseUp={() => handleMouseUp(slotIndex)}
                                >
                                    {slotIndex < 6 && (
                                        <Box
                                            style={{
